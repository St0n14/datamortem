...
@celery_app.task(name="parse_registry_task")
def parse_registry_task(evidence_uid: str, task_run_id: int):
    db = SessionLocal()
    ...
    try:
        out_dir = f"/lake/{ev.case.case_id}/registry/{evidence_uid}"
        os.makedirs(out_dir, exist_ok=True)
        output_path = os.path.join(out_dir, "part-000.parquet")

        with open(output_path, "w") as f:
            f.write("registry_placeholder\n")
